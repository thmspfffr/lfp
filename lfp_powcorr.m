...
%   
  ...
  rel_depth = [min([d(out.exp_num).rel_depth]):25:max([d(out.exp_num).rel_depth])];
  indiv_depth_idx = ~isnan(d(out.exp_num).rel_depth);
  ...
  out.coh = nan(size(all_depth(:),1),size(all_depth(:),1),length(freqs));
  out.powcorr = nan(size(all_depth(:),1),size(all_depth(:),1),length(freqs));
  
  for ifoi = 1 : length(freqs)
    fprintf('Computing icoh for freq%d...\n',ifoi)

    idx = f>=freqs(ifoi,1)&f<=freqs(ifoi,2);
    coh(:,:,ifoi) = mean(tmp_coh(:,:,idx),3);  
    ...
    powcorr(:,:,ifoi) = compute_orthopowcorr(ampenv');
  
  end
  
  out.coh(abs_depth_idx,abs_depth_idx,:)      = coh(indiv_depth_idx,indiv_depth_idx,:);
  out.powcorr(abs_depth_idx,abs_depth_idx,:)  = dfa(indiv_depth_idx,indiv_depth_idx,:);
